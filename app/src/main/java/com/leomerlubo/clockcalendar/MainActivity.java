...
public class MainActivity extends Activity {
    ...
    int bg, fg, accent;
    boolean analog, month, seconds, twentyFour, calendarScrollable, showBattery;
    int fontChoice;
    float brightness;
    ...
    class ClockView extends View {
        ...
        ClockView(Context c,boolean preview){super(c);this.preview=preview;setBackgroundColor(bg);readBattery();if(!preview)timer.post(tick);setOnTouchListener((v,e)->{if(preview)return true;if(e.getAction()==MotionEvent.ACTION_DOWN){downX=e.getX();downY=e.getY();downAt=System.currentTimeMillis();return true;}if(e.getAction()==MotionEvent.ACTION_UP){float upX=e.getX(),dx=upX-downX;if(downX>getWidth()-dp(70)&&downY<dp(70)&&System.currentTimeMillis()-downAt>700){exitMenu();downAt=0;return true;}if(calendarScrollable&&inCalendar(downY)){if(Math.abs(dx)>dp(35))moveCalendar(dx<0?1:-1);else if(downX<getWidth()*.30f)moveCalendar(-1);else if(downX>getWidth()*.70f)moveCalendar(1);}downAt=0;return true;}return true;});}
        void resetDisplayDate(){displayDate=(Calendar)now.clone();}
        boolean inCalendar(float touchY){return getWidth()>getHeight()?downX>getWidth()*.52f:touchY>getHeight()*.49f;}
