...
public class MainActivity extends Activity {
    ...
    int bg, fg, accent;
    boolean analog, month, seconds, twentyFour, calendarScrollable, showBattery, clockFirst;
    int fontChoice;
    float brightness;
    ...
    class ClockView extends View {
        ...
        @Override protected void onDraw(Canvas c){super.onDraw(c);now=Calendar.getInstance();c.save();if(!preview)c.translate(dp(shiftX),dp(shiftY));boolean land=getWidth()>getHeight();float split=land?getWidth()*.52f:getHeight()*.49f;
            if(land){float clockW=split,calW=getWidth()-clockW,divider=clockFirst?clockW:calW;if(clockFirst){drawClock(c,0,0,clockW,getHeight());drawCalendar(c,clockW,0,calW,getHeight());}else{drawCalendar(c,0,0,calW,getHeight());drawClock(c,calW,0,clockW,getHeight());}line(c,divider,getHeight()*.14f,divider,getHeight()*.86f);}else{float clockH=split,calH=getHeight()-clockH,divider=clockFirst?clockH:calH;if(clockFirst){drawClock(c,0,0,getWidth(),clockH);drawCalendar(c,0,clockH,getWidth(),calH);}else{drawCalendar(c,0,0,getWidth(),calH);drawClock(c,0,calH,getWidth(),clockH);}line(c,getWidth()*.12f,divider,getWidth()*.88f,divider);}if(!preview){font(dp(20),blend(fg,bg,.22f),Paint.Align.CENTER);c.drawText("×",getWidth()-dp(30),dp(38),p);}c.restore();}
