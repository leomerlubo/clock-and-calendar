...
public class MainActivity extends Activity {
    ...
    int bg, fg, accent;
    boolean analog, month, seconds, twentyFour, calendarScrollable, showBattery, clockFirst, clockOnly;
    int fontChoice;
    float brightness;
    ...
    void load() {
        ...
        calendarScrollable = prefs.getBoolean("calendarScrollable", false);
        showBattery = prefs.getBoolean("showBattery", true);
        clockFirst = prefs.getBoolean("clockFirst", true);
        clockOnly = prefs.getBoolean("clockOnly", false);
    ...
    void save() {
        prefs.edit().putBoolean("configured", true).putInt("bg", bg).putInt("fg", fg)
            ...
            .putBoolean("calendarScrollable", calendarScrollable).putBoolean("showBattery", showBattery)
            .putBoolean("clockFirst", clockFirst)
            .putBoolean("clockOnly", clockOnly)
    ...
    public void showSetup() {
        ...
        space(page, 12); page.addView(toggleRow("Clock only", clockOnly, b -> {clockOnly=b; preview.invalidate();}));
        ...
        space(page, 12); LinearLayout calMotion = segmented("Fixed", "Scrollable", !calendarScrollable, v -> { calendarScrollable = v == 1; preview.resetDisplayDate(); preview.invalidate(); }); page.addView(calMotion);
        space(page, 18); addLabel(page, "LAYOUT"); LinearLayout positionChoice = segmented("Clock left / top", "Clock right / bottom", clockFirst, v -> { clockFirst = v == 0; preview.invalidate(); }); page.addView(positionChoice);
        ...
        page.addView(toggleRow("Show battery level", showBattery, b -> {showBattery=b; preview.invalidate();}));
    ...
    class ClockView extends View {
        Paint p=new Paint(3); Calendar now=Calendar.getInstance(), displayDate=Calendar.getInstance(); boolean preview; Handler timer=new Handler(Looper.getMainLooper()); float downX,downY; long downAt; int shiftX,shiftY, batteryLevel=0, chargePhase=0; boolean charging;
        Runnable tick=new Runnable(){public void run(){now=Calendar.getInstance();readBattery();if(charging)chargePhase=(chargePhase+1)%5;int slot=(int)(System.currentTimeMillis()/300000L);shiftX=(slot%5)-2;shiftY=((slot/5)%5)-2;invalidate();timer.postDelayed(this,charging?450:1000);}};
        ClockView(Context c,boolean preview){super(c);this.preview=preview;setBackgroundColor(bg);readBattery();if(!preview)timer.post(tick);setOnTouchListener((v,e)->{if(preview)return true;if(e.getAction()==MotionEvent.ACTION_DOWN){downX=e.getX();downY=e.getY();downAt=System.currentTimeMillis();return true;}if(e.getAction()==MotionEvent.ACTION_UP){float upX=e.getX(),dx=upX-downX;if(downX>getWidth()-dp(70)&&downY<dp(70)&&System.currentTimeMillis()-downAt>700){exitMenu();downAt=0;return true;}if(calendarScrollable&&inCalendar(downX,downY)){float fraction=calendarXFraction(downX);if(Math.abs(dx)>dp(35))moveCalendar(dx<0?1:-1);else if(isTodayButton(downY,fraction))resetDisplayDate();else if(fraction<.25f)moveCalendar(-1);else if(fraction>.75f)moveCalendar(1);}downAt=0;return true;}return true;});}
        void resetDisplayDate(){displayDate=(Calendar)now.clone();invalidate();}
        boolean inCalendar(float touchX,float touchY){if(getWidth()>getHeight()){float clockW=getWidth()*.52f;return clockFirst?touchX>clockW:touchX<getWidth()-clockW;}float clockH=getHeight()*.49f;return clockFirst?touchY>clockH:touchY<getHeight()-clockH;}
        float calendarXFraction(float touchX){if(getWidth()<=getHeight())return touchX/getWidth();float clockW=getWidth()*.52f,calW=getWidth()-clockW,calLeft=clockFirst?clockW:0;return (touchX-calLeft)/calW;}
        boolean isTodayButton(float touchY,float fraction){float clockH=getHeight()*.49f,calTop=getWidth()>getHeight()?0:(clockFirst?clockH:0),calH=getWidth()>getHeight()?getHeight():getHeight()-clockH;float localY=(touchY-calTop)/calH;return fraction>=.25f&&fraction<=.75f&&localY>=.02f&&localY<=.18f;}
        void moveCalendar(int direction){if(month)displayDate.add(Calendar.MONTH,direction);else displayDate.add(Calendar.DAY_OF_MONTH,direction);invalidate();}
        void readBattery(){Intent i=registerReceiver(null,new IntentFilter(Intent.ACTION_BATTERY_CHANGED));if(i==null)return;int level=i.getIntExtra(BatteryManager.EXTRA_LEVEL,0),scale=i.getIntExtra(BatteryManager.EXTRA_SCALE,100),status=i.getIntExtra(BatteryManager.EXTRA_STATUS,-1);batteryLevel=scale>0?Math.round(level*100f/scale):0;charging=status==BatteryManager.BATTERY_STATUS_CHARGING||status==BatteryManager.BATTERY_STATUS_FULL;}
        ...
        @Override protected void onDraw(Canvas c){super.onDraw(c);now=Calendar.getInstance();c.save();if(!preview)c.translate(dp(shiftX),dp(shiftY));if(clockOnly){drawClockOnly(c);}else{boolean land=getWidth()>getHeight();float split=land?getWidth()*.52f:getHeight()*.49f;
            if(land){float clockW=split,calW=getWidth()-clockW,divider=clockFirst?clockW:calW;if(clockFirst){drawClock(c,0,0,clockW,getHeight());drawCalendar(c,clockW,0,calW,getHeight());}else{drawCalendar(c,0,0,calW,getHeight());drawClock(c,calW,0,clockW,getHeight());}line(c,divider,getHeight()*.14f,divider,getHeight()*.86f);}else{float clockH=split,calH=getHeight()-clockH,divider=clockFirst?clockH:calH;if(clockFirst){drawClock(c,0,0,getWidth(),clockH);drawCalendar(c,0,clockH,getWidth(),calH);}else{drawCalendar(c,0,0,getWidth(),calH);drawClock(c,0,calH,getWidth(),clockH);}line(c,getWidth()*.12f,divider,getWidth()*.88f,divider);}}
            if(!preview){font(dp(20),blend(fg,bg,.22f),Paint.Align.CENTER);c.drawText("×",getWidth()-dp(30),dp(38),p);}c.restore();}
        void line(Canvas c,float a,float b,float x,float y){p.setColor(blend(fg,bg,.11f));p.setStrokeWidth(dp(1));c.drawLine(a,b,x,y,p);}
        void drawClockOnly(Canvas c){if(analog)drawAnalogOnly(c,0,0,getWidth(),getHeight());else drawDigitalOnly(c,0,0,getWidth(),getHeight());}
        void drawDigitalOnly(Canvas c,float x,float y,float w,float h){String pattern=twentyFour?(seconds?"HH:mm:ss":"HH:mm"):(seconds?"h:mm:ss":"h:mm");String time=new SimpleDateFormat(pattern,Locale.getDefault()).format(now.getTime());float size=Math.min(w/(time.length()*.56f),h*.55f);font(size,fg,Paint.Align.CENTER);float timeY=y+h*(showBattery?.51f:.57f);c.drawText(time,x+w/2,timeY,p);String am=twentyFour?"":new SimpleDateFormat("a",Locale.getDefault()).format(now.getTime());float statusSize=Math.max(dp(22),Math.min(size*.20f,h*.10f));float statusY=y+h*.70f;if(showBattery&&!twentyFour){float gap=dp(22),batteryTotal=batteryTotalWidth(statusSize),amWidth=measure(am,statusSize,.14f),total=amWidth+gap+batteryTotal,start=x+(w-total)/2;font(statusSize,blend(fg,bg,.58f),Paint.Align.LEFT);p.setLetterSpacing(.14f);c.drawText(am,start,statusY,p);p.setLetterSpacing(0);drawBattery(c,start+amWidth+gap,statusY-statusSize*.37f,statusSize,true);}else if(!twentyFour){font(statusSize,blend(fg,bg,.58f),Paint.Align.CENTER);p.setLetterSpacing(.14f);c.drawText(am,x+w/2,statusY,p);p.setLetterSpacing(0);}else if(showBattery)drawBattery(c,x+w/2,statusY-statusSize*.37f,statusSize);}
        void drawAnalogOnly(Canvas c,float x,float y,float w,float h){float batteryRoom=showBattery?h*.14f:0;float availableH=h-batteryRoom;float cx=x+w/2,cy=y+availableH*.50f,r=Math.min(w*.42f,availableH*.40f);drawAnalogFace(c,cx,cy,r);if(showBattery)drawBattery(c,x+w/2,y+h*.88f,Math.min(dp(24),h*.06f));}
        void drawClock(Canvas c,float x,float y,float w,float h){if(analog){drawAnalog(c,x,y,w,h);if(showBattery)drawBattery(c,x+w/2,y+h*.87f,Math.min(dp(12),h*.035f));}else drawDigital(c,x,y,w,h);}
        void drawDigital(Canvas c,float x,float y,float w,float h){String pattern=twentyFour?(seconds?"HH:mm:ss":"HH:mm"):(seconds?"h:mm:ss":"h:mm");String time=new SimpleDateFormat(pattern,Locale.getDefault()).format(now.getTime());float size=Math.min(w/(time.length()*.57f),h*.40f);font(size,fg,Paint.Align.CENTER);c.drawText(time,x+w/2,y+h*.54f,p);String am=twentyFour?"":new SimpleDateFormat("a",Locale.getDefault()).format(now.getTime());float statusSize=Math.max(dp(18),size*.21f);float cy=y+h*.70f;if(showBattery&&!twentyFour){float gap=dp(18),batteryTotal=batteryTotalWidth(statusSize),amWidth=measure(am,statusSize,.14f),total=amWidth+gap+batteryTotal,start=x+(w-total)/2;font(statusSize,blend(fg,bg,.58f),Paint.Align.LEFT);p.setLetterSpacing(.14f);c.drawText(am,start,cy,p);p.setLetterSpacing(0);drawBattery(c,start+amWidth+gap,cy-statusSize*.37f,statusSize,true);}else if(!twentyFour){font(statusSize,blend(fg,bg,.58f),Paint.Align.CENTER);p.setLetterSpacing(.14f);c.drawText(am,x+w/2,cy,p);p.setLetterSpacing(0);}else if(showBattery)drawBattery(c,x+w/2,cy-statusSize*.37f,statusSize);}
        float measure(String value,float size,float spacing){font(size,fg,Paint.Align.LEFT);p.setLetterSpacing(spacing);float result=p.measureText(value);p.setLetterSpacing(0);return result;}
        float batteryTotalWidth(float size){return size*4.85f;}
        void drawBattery(Canvas c,float centerX,float centerY,float size){drawBattery(c,centerX-batteryTotalWidth(size)/2,centerY,size,true);}
        void drawBattery(Canvas c,float left,float centerY,float size,boolean fromLeft){float bodyH=size*.82f,bodyW=size*2.55f,top=centerY-bodyH/2,stroke=Math.max(dp(1),size*.075f);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(stroke);p.setColor(blend(fg,bg,.58f));p.setStrokeJoin(Paint.Join.ROUND);RectF shell=new RectF(left,top,left+bodyW,top+bodyH);c.drawRoundRect(shell,bodyH*.22f,bodyH*.22f,p);p.setStyle(Paint.Style.FILL);c.drawRoundRect(new RectF(left+bodyW+stroke,centerY-bodyH*.19f,left+bodyW+stroke+size*.16f,centerY+bodyH*.19f),size*.05f,size*.05f,p);float innerLeft=left+size*.22f,barGap=size*.11f,barW=size*.39f,barH=bodyH*.58f;int filled=Math.min(4,(int)Math.ceil(batteryLevel/25f));for(int i=0;i<4;i++){float bx=innerLeft+i*(barW+barGap);float alpha=i<filled?.78f:.16f;if(charging){int lit=chargePhase==0?4:chargePhase;alpha=i<lit?.92f:.13f;}p.setColor(blend(fg,bg,alpha));c.drawRoundRect(new RectF(bx,centerY-barH/2,bx+barW,centerY+barH/2),barW*.32f,barW*.32f,p);}font(size*.78f,blend(fg,bg,.58f),Paint.Align.LEFT);c.drawText(batteryLevel+"%",left+bodyW+size*.62f,centerY+size*.27f,p);}
        void drawAnalog(Canvas c,float x,float y,float w,float h){float cx=x+w/2,cy=y+h/2,r=Math.min(w,h)*.31f;drawAnalogFace(c,cx,cy,r);}
        void drawAnalogFace(Canvas c,float cx,float cy,float r){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(dp(1));p.setColor(blend(fg,bg,.2f));c.drawCircle(cx,cy,r,p);for(int i=0;i<60;i++){double a=i*Math.PI/30-Math.PI/2;float in=r*(i%5==0?.88f:.95f);p.setColor(i%5==0?blend(fg,bg,.65f):blend(fg,bg,.22f));p.setStrokeWidth(i%5==0?dp(2):dp(1));c.drawLine(cx+(float)Math.cos(a)*in,cy+(float)Math.sin(a)*in,cx+(float)Math.cos(a)*r,cy+(float)Math.sin(a)*r,p);}float min=now.get(Calendar.MINUTE)+now.get(Calendar.SECOND)/60f;float hr=now.get(Calendar.HOUR)+min/60f;hand(c,cx,cy,r*.54f,hr*Math.PI/6-Math.PI/2,fg,dp(4));hand(c,cx,cy,r*.76f,min*Math.PI/30-Math.PI/2,fg,dp(3));if(seconds)hand(c,cx,cy,r*.76f,now.get(Calendar.SECOND)*Math.PI/30-Math.PI/2,accent,dp(1));p.setStyle(Paint.Style.FILL);p.setColor(accent);c.drawCircle(cx,cy,dp(4),p);}
        ...
        void drawCalendar(Canvas c,float x,float y,float w,float h){Calendar shown=calendarScrollable?displayDate:now;if(month)drawMonth(c,x,y,w,h,shown);else drawToday(c,x,y,w,h,shown);}
        void nav(Canvas c,float x,float y,float w,float baseline){if(!calendarScrollable)return;float size=Math.min(dp(14),w*.036f);font(size,blend(fg,bg,.52f),Paint.Align.CENTER);p.setLetterSpacing(.08f);c.drawText("‹‹",x+w*.14f,baseline,p);c.drawText("TODAY",x+w*.50f,baseline,p);c.drawText("››",x+w*.86f,baseline,p);p.setLetterSpacing(0);}
        void drawToday(Canvas c,float x,float y,float w,float h,Calendar shown){String week=new SimpleDateFormat("EEEE",Locale.getDefault()).format(shown.getTime()).toUpperCase();String mon=new SimpleDateFormat("MMMM",Locale.getDefault()).format(shown.getTime()).toUpperCase();nav(c,x,y,w,y+h*.105f);font(Math.min(dp(13),w*.032f),blend(fg,bg,.48f),Paint.Align.CENTER);p.setLetterSpacing(.22f);c.drawText(week,x+w/2,y+h*.30f,p);p.setLetterSpacing(0);font(Math.min(w*.24f,h*.34f),fg,Paint.Align.CENTER);c.drawText(String.valueOf(shown.get(Calendar.DAY_OF_MONTH)),x+w/2,y+h*.61f,p);font(Math.min(dp(13),w*.034f),accent,Paint.Align.CENTER);p.setLetterSpacing(.18f);c.drawText(mon+"  "+shown.get(Calendar.YEAR),x+w/2,y+h*.76f,p);p.setLetterSpacing(0);}
        void drawMonth(Canvas c,float x,float y,float w,float h,Calendar shown){Calendar first=(Calendar)shown.clone();first.set(Calendar.DAY_OF_MONTH,1);String title=new SimpleDateFormat("MMMM yyyy",Locale.getDefault()).format(shown.getTime());float navTop=y+h*.105f,titleTop=y+h*(calendarScrollable?.205f:.17f);nav(c,x,y,w,navTop);font(Math.min(dp(20),w*.055f),fg,Paint.Align.CENTER);c.drawText(title,x+w*.50f,titleTop,p);String[] days={"S","M","T","W","T","F","S"};float left=x+w*.12f,right=x+w*.88f,cw=(right-left)/7f;float head=y+h*(calendarScrollable?.325f:.30f);for(int i=0;i<7;i++){font(Math.min(dp(10),cw*.27f),blend(fg,bg,.35f),Paint.Align.CENTER);c.drawText(days[i],left+cw*(i+.5f),head,p);}int start=first.get(Calendar.DAY_OF_WEEK)-1,max=first.getActualMaximum(Calendar.DAY_OF_MONTH);float rowH=h*(calendarScrollable?.096f:.105f);boolean currentMonth=shown.get(Calendar.YEAR)==now.get(Calendar.YEAR)&&shown.get(Calendar.MONTH)==now.get(Calendar.MONTH);for(int d=1;d<=max;d++){int pos=start+d-1,col=pos%7,row=pos/7;float cx=left+cw*(col+.5f),cy=head+rowH*(row+1);if(currentMonth&&d==now.get(Calendar.DAY_OF_MONTH)){p.setColor(accent);p.setStyle(Paint.Style.FILL);c.drawCircle(cx,cy-dp(4),Math.min(dp(16),cw*.34f),p);font(Math.min(dp(12),cw*.34f),bg,Paint.Align.CENTER);}else font(Math.min(dp(12),cw*.34f),fg,Paint.Align.CENTER);c.drawText(String.valueOf(d),cx,cy,p);}}
